...
public class RestaurantOrders 
{
    ...
    public boolean isLate() {
        ...
        try {
            LocalDateTime placingOrderDate = LocalDateTime.parse(placing_order_date, formatter);
            LocalDateTime orderReceivedDate = LocalDateTime.parse(order_received, formatter);

            Duration duration = Duration.between(placingOrderDate, orderReceivedDate);
            return duration.toHours() >= 1; // Consider the order late if it's an hour or more late
        } catch (Exception e) {
            System.out.println("Error parsing date: " + e.getMessage());
            return false; // If there's an error parsing the date, assume the order is not late
        }
    }
